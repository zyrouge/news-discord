...
class Client {
    ...
    initialize() {
        return __awaiter(this, void 0, void 0, function* () {
            Utils_1.Logger.log(`Loading Events from ${Utils_1.Logger.chalk.underline(this.options.events)}`);
            ...
            for (const file of eventFiles) {
                ...
                Utils_1.Logger.debug(`Event loaded: ${Utils_1.Logger.chalk.underline(file)}`);
            }
            Utils_1.Logger.log(`Loading Commands from ${Utils_1.Logger.chalk.underline(this.options.events)}`);
            const commandFiles = yield fs_1.promises.readdir(this.options.commands);
            for (const file of commandFiles) {
                this.commander.load(file);
                Utils_1.Logger.debug(`Command loaded: ${Utils_1.Logger.chalk.underline(file.split(".")[0])}`);
            }
        });
    }
    finalize() {
